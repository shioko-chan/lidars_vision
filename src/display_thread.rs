use anyhow::anyhow;
use intelligent_sight_lib::{ImageBuffer, Searcher, TensorBuffer, UnifiedTrait};
use log::error;
...
pub struct DisplayThread {
    image_searcher: Searcher<ImageBuffer>,
        ...
        thread::spawn(move || {
            while self.stop_sig.load(Ordering::Relaxed) == false {
                ...
                if let Ok(detection) = self.detection_rx.recv() {
                    ...
                    if missing % 10 == 0 {
                        error!("missing rate {}", missing as f32 / cnt as f32);
                    }

                    let Some(mut image) = self.image_searcher.search(detection.timestamp) else {
                        missing += 1;
                        continue;
                    };
