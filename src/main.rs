...
pub fn get_config() -> Option<&'static AppConfig> {
    unsafe { std::ptr::addr_of!(CONFIG).as_ref().unwrap().as_ref() }
}
...
fn main() {
    ...
    let classification_thread = match classification_thread::ClassificationThread::new(
        postprocess_thread.get_output_buffer(),
        camera_thread.get_searcher(),
        stop_sig.clone(),
    ) {
        Ok(classification_thread) => classification_thread,
        Err(err) => {
            error!("Main: Failed to initialize Classification thread: {}", err);
            return;
        }
    };
    ...
    #[cfg(feature = "visualize")]
    let display_thread =
        display_thread::DisplayThread::new(camera_thread.get_searcher(), rx_det, stop_sig.clone());
    ...
    // let classification_thread_handle = classification_thread.start_processor();
    info!("Main: Classification thread started");
    ...
    // classification_thread_handle.join().unwrap();
