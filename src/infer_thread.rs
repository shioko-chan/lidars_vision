...
impl Drop for TrtThread {
    fn drop(&mut self) {
        if let Err(err) = release_resources(0) {
...
impl Processor for TrtThread {
    ...
    fn start_processor(self) -> thread::JoinHandle<()> {
        thread::spawn(move || {
            if let Err(err) = create_context(0) {
                error!("InferThread: failed create context, due to error: {}", err);
                self.stop_sig.store(true, Ordering::Relaxed);
                return;
            }

            let mut engine_input_buffer = TensorBuffer::new(vec![1, 3, 1280, 1024]).unwrap();
            if let Err(err) = set_input(0, &mut engine_input_buffer) {
            ...
            while self.stop_sig.load(Ordering::Relaxed) == false {
                ...
                if let Err(err) = set_output(0, &mut lock_output) {
                    error!("InferThread: set output buffer failed, error {}", err);
                    break;
                }
                if let Err(err) = infer(0) {
