...
mod gpu_op_ffi {
    ...
    extern "C" {
        ...
        pub fn transfer_host_to_host(dst_buffer: *mut u8, src_buffer: *const u8, size: u32) -> u16;
        ...
        pub fn convert_rgb888_3dtensor_with_padding(
            input_buffer: *const u8,
            output_buffer: *mut f32,
            width: u32,
            height: u32,
        ...
        pub fn create_context(id: u8) -> u16;
        pub fn infer(id: u8) -> u16;
        pub fn release_resources(id: u8) -> u16;
        pub fn set_input(id: u8, input_buffer: *mut f32) -> u16;
        pub fn set_output(id: u8, output_buffer: *mut f32) -> u16;
...
pub fn convert_rgb888_3dtensor_with_padding(
    ...
    exec_and_check(|| {
        Ok(unsafe {
            gpu_op_ffi::convert_rgb888_3dtensor_with_padding(
                input_image.to_device()?,
                output_tensor.device()?,
                input_image.width,
                input_image.height,
...
pub fn transfer_host_to_host<T>(dst: *mut T, src: *const T, size: usize) -> Result<()> {
    exec_and_check(|| {
        Ok(unsafe {
            gpu_op_ffi::transfer_host_to_host(
                dst as *mut u8,
                src as *const u8,
                (size * mem::size_of::<T>() / mem::size_of::<u8>()) as u32,
            )
...
pub fn create_context(id: u8) -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::create_context(id) }))
}

pub fn infer(id: u8) -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::infer(id) }))
}

pub fn release_resources(id: u8) -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::release_resources(id) }))
}

pub fn set_input(id: u8, input_buffer: &mut UnifiedItem<f32>) -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::set_input(id, input_buffer.to_device()?) }))
}

pub fn set_output(id: u8, output_buffer: &mut UnifiedItem<f32>) -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::set_output(id, output_buffer.device()?) }))
}
...
pub fn postprocess_classify(
    ...
    exec_and_check(|| {
        Ok(unsafe {
            gpu_op_ffi::postprocess_classify(input_buffer.device()?, feature_map_size, &mut cls)
        })
    })
    .map(|_| cls)
}

pub fn postprocess_classify_destroy() -> Result<()> {
    exec_and_check(|| Ok(unsafe { gpu_op_ffi::postprocess_classify(null(), 0, null_mut()) }))
}
