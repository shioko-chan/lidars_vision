...
uint16_t PostProcess::post_process(float *input_buffer, float *output_buffer, uint16_t *num_detections)
{
    ...
    check_status(cudaMemcpy(this->host_transformed, this->transformed, FEATURE_MAP_SIZE * 5 * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (int j = 0; i < MAX_DETECT && j != -1; ++i)
    {
        ...
        {
            output_buffer[i * 5 + item] = this->host_transformed[idx * 5 + item];
        }

        int next = -1;
        float *box = this->host_transformed + idx * 5;
        for (; j < end; ++j)
        {
            ...
            if (check_iou(box, this->host_transformed + idx1 * 5))
            {
...
uint16_t postprocess_classify(float *input_buffer, uint16_t feature_map_size, uint16_t *cls)
{
    ...
    {
        check_status(cudaFree(TRANSFORMED));
        check_status(cudaFree(INDICES));
        return (uint16_t)cudaSuccess;
    }
    ...
    thrust::device_ptr<float> d_TRANSFORMED = thrust::device_ptr<float>(TRANSFORMED);
    thrust::device_ptr<int> d_INDICES = thrust::device_ptr<int>(INDICES);
    ...
    thrust::sequence(d_INDICES, d_INDICES + feature_map_size);
    thrust::sort(d_INDICES, d_INDICES + feature_map_size, [d_TRANSFORMED] __device__(int a, int b)
