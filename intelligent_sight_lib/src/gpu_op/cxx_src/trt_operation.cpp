...
TensorrtInfer *TRT_INFER[2] = {nullptr};
...
{
    check_status(cudaStreamDestroy(CUDA_STREAM));
    return (uint16_t)TRT_OK;
}

uint16_t create_engine(uint8_t id, const char *engine_filename, const char *input_name, const char *output_name, uint32_t width, uint32_t height)
{
    if (TRT_INFER[id] != nullptr)
    {
        return TRT_ENGINE_ALREADY_CREATED;
    }
    TRT_INFER[id] = new TensorrtInfer(engine_filename, input_name, output_name, width, height);
    return TRT_INFER[id]->create_engine();
}

uint16_t create_context(uint8_t id)
{
    if (TRT_INFER[id] != nullptr)
    {
        return TRT_INFER[id]->create_context();
    }
    else
    {
        return TRT_ENGINE_NOT_INITIALIZED;
...
uint16_t set_input(uint8_t id, float *input_buffer)
{
    return TRT_INFER[id]->set_input(input_buffer);
}
uint16_t set_output(uint8_t id, float *output_buffer)
{
    return TRT_INFER[id]->set_output(output_buffer);
}

uint16_t infer(uint8_t id)
{
    return TRT_INFER[id]->infer();
}

uint16_t release_resources(uint8_t id)
{
    check_status(TRT_INFER[id]->release_resources());
    delete TRT_INFER[id];
    return TRT_OK;
}
